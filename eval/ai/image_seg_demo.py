"""ALAS — single-image segmentation demo.

Runs segmentation on one or more image files and displays the result. With
--save, the overlay, mask, and combined views are written under
...
# Resolve the repository root so inputs/outputs use fixed locations regardless
# of the current working directory.
_REPO_ROOT = next(p for p in Path(__file__).resolve().parents if (p / "src").is_dir())
_SAMPLES_DIR = _REPO_ROOT / "eval" / "ai" / "samples"
...
def run_image(model_path, image_path, input_h=384, input_w=512, save=False):
    ...
    if save:
        ...
        out_dir = _REPO_ROOT / "outputs" / "eval" / "ai" / "segmentation_samples"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--images", nargs='+', required=False,
                        ...
                        default=[
                            str(_SAMPLES_DIR / "test1.jpeg"),
                            str(_SAMPLES_DIR / "test2.jpeg"),
                            str(_SAMPLES_DIR / "test3.jpeg"),
                            str(_SAMPLES_DIR / "sample_uuid.jpeg"),
                        ])
